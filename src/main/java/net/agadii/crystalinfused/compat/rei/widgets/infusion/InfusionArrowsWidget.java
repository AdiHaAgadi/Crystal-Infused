...
public class InfusionArrowsWidget extends BoundedWidget {
    private Rectangle bounds;
    private double animationDuration = -1;
    private final Identifier TEXTURE = new Identifier(CrystalInfused.MOD_ID, "textures/gui/rei/infusion_arrows.png");
    private final NumberAnimator<Float> darkBackgroundAlpha = ValueAnimator.ofFloat()
            .withConvention(() -> REIRuntime.getInstance().isDarkThemeEnabled() ? 1.0F : 0.0F, ValueAnimator.typicalTransitionTime())
            .asFloat();

    public InfusionArrowsWidget(Rectangle bounds) {
    ...
    public void renderBackground(DrawContext context, boolean dark, float alpha) {
        ...
        if (getAnimationDuration() > 0) {
            int height = MathHelper.ceil((System.currentTimeMillis() / (animationDuration / 18) % 18d));
            context.drawTexture(TEXTURE, getX(), getY(), 0, 0, 14, height);
        } else {
            context.drawTexture(TEXTURE, getX(), getY(), 0, 0, 14, 18);
        }
